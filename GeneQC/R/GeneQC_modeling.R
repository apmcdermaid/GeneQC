GeneQC_modeling <- function(dat){
  ...
  list3 <- list(d, sd, THETAS[,order(apply(FUN =  gamma_mode1,MARGIN =  2, X = THETAS))], PARS[,order(PARS[1,])], cutoffs, parameters, categ, sign, .res[,c(1:4,6:8)], .res)
}
